...
# Set Title
st.title('Network Graph - Keywords and Ticker Count')
...
if sel_date:
    ...
    sel_data['ticker count'] = sel_data['ticker count'].map(lambda x: literal_eval(x))
    

    edge = pd.DataFrame()
    for idx in range(len(sel_data)):
        tmp = sel_data['ticker count'][idx]

        df = pd.DataFrame(tmp, columns = ['ticker', 'count'])
        df['keyword'] = sel_data['keyword'][idx]
        df = df[['keyword', 'ticker', 'count']]
        edge = pd.concat([edge, df]).reset_index(drop=True)

    st.dataframe(edge)
    edge_list = []
    edge.apply(lambda x: edge_list.append((x['keyword'], x['ticker'], x['count'])), axis=1)
    ...
    if sel_chart:
        st.pyplot(fig)
        if sel_save:
            plt.savefig(f'img/KEY_{sel_date}_TOP{sel_head}_img.{sel_save.lower()}')
